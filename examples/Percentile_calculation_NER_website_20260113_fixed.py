...
PBCD_XPT_PATH = Path(__file__).with_name("PbCd_h.XPT")
DEMO_XPT_PATH = Path(__file__).with_name("demo_h.XPT")
...
def _weighted_mean_and_se_by_psu(df, value_col, weight_col, strata_col, psu_col):
    ...
    # SUDAAN NSUM is the unweighted number of valid observations in the subpopulation,
    # not the sum of the 0/1 indicator.
    nsum = float(len(work))

    total_w = work[weight_col].sum()
    mean = (work[value_col] * work[weight_col]).sum() / total_w if total_w > 0 else np.nan

    psu_est = (
        work.groupby([strata_col, psu_col], dropna=False)[[value_col, weight_col]]
        .apply(lambda g: (g[value_col] * g[weight_col]).sum() / g[weight_col].sum() if g[weight_col].sum() > 0 else np.nan)
        .reset_index(name="_psu_mean")
    )

    strata_stats = psu_est.groupby(strata_col, dropna=False)["_psu_mean"].agg(["count", "mean", "var"]).reset_index()
    valid = strata_stats["count"] >= 2

    var_mean = 0.0
    for _, row in strata_stats[valid].iterrows():
        nh = row["count"]
        sh2 = 0.0 if pd.isna(row["var"]) else row["var"]
        var_mean += sh2 / nh

    semean = np.sqrt(var_mean) if var_mean >= 0 else np.nan

    p = mean
    srs_var = p * (1.0 - p) / nsum if nsum > 0 and pd.notna(p) else np.nan
    deffmean = var_mean / srs_var if pd.notna(srs_var) and srs_var > 0 else np.nan

    atlev1 = float(psu_est[strata_col].nunique())
    atlev2 = float(len(psu_est))
...
def pcntci(var1, var2, var3, var4, var5, var6, var7, L6dem):
    ...
    # ***Step 4;
    xXtest = xxCYTO.copy()
    L95_macro = float(xXtest["L95"].iloc[0])
    U95_macro = float(xXtest["U95"].iloc[0])
    MEAN_macro = float(xXtest["PT"].iloc[0])
    xXtest["mvar"] = 1
...
# run;
print(allperc)
allperc.to_csv(Path(__file__).with_name("allperc_python_fixed.csv"), index=False)
