...
function buildPythonExecutionEnvelope(code: string) {
  ...
  return [
    ...
    "            continue",
    "    return __artifacts",
    "def __sas2py_new_output_files(__before, __suffixes, __Path=__sas2py_Path):",
    "    __paths = []",
    "    __root = __Path.cwd()",
    "    for __path in sorted(__root.rglob('*')):",
    "        try:",
    ...
    "            if any(__part in {'.git', '__pycache__'} for __part in __path.parts):",
    "                continue",
    "            if __path.suffix.lower() not in __suffixes:",
    "                continue",
    "            __relative = __path.relative_to(__root).as_posix()",
    "            __stat = __path.stat()",
    "            if __stat.st_size <= 0:",
    "                continue",
    "            if __before.get(__relative) == (__stat.st_size, __stat.st_mtime_ns):",
    "                continue",
    "            __paths.append(__path)",
    "        except Exception:",
    "            continue",
    "    return __paths",
    "def __sas2py_sheet_name(__name, __used):",
    "    __clean = ''.join((__char if __char not in '[]:*?/\\\\' else '_') for __char in __name).strip()",
    "    if not __clean:",
    "        __clean = 'Sheet'",
    "    __clean = __clean[:31]",
    "    __candidate = __clean",
    "    __index = 2",
    "    while __candidate in __used:",
    "        __suffix = '_' + str(__index)",
    "        __candidate = __clean[:31 - len(__suffix)] + __suffix",
    "        __index += 1",
    "    __used.add(__candidate)",
    "    return __candidate",
    "def __sas2py_create_combined_csv_workbook(__before, __Path=__sas2py_Path):",
    "    try:",
    "        import pandas as __sas2py_pd",
    "    except Exception:",
    "        return",
    "    __csv_paths = __sas2py_new_output_files(__before, {'.csv'})",
    "    if len(__csv_paths) < 2:",
    "        return",
    "    if __sas2py_new_output_files(__before, {'.xls', '.xlsx'}):",
    "        return",
    "    __root = __Path.cwd()",
    "    __workbook_path = __root / 'sas2py_outputs.xlsx'",
    "    __index = 2",
    "    while __workbook_path.exists():",
    "        __workbook_path = __root / ('sas2py_outputs_' + str(__index) + '.xlsx')",
    "        __index += 1",
    "    for __engine in ('openpyxl', 'xlsxwriter'):",
    "        try:",
    "            __used_sheets = set()",
    "            __written = 0",
    "            with __sas2py_pd.ExcelWriter(__workbook_path, engine=__engine) as __writer:",
    "                for __csv_path in __csv_paths:",
    "                    try:",
    "                        __df = __sas2py_pd.read_csv(__csv_path)",
    "                        __sheet = __sas2py_sheet_name(__csv_path.stem, __used_sheets)",
    "                        __df.to_excel(__writer, sheet_name=__sheet, index=False)",
    "                        __written += 1",
    "                    except Exception:",
    "                        continue",
    "            if __written > 0 and __workbook_path.exists() and __workbook_path.stat().st_size > 0:",
    "                return",
    "        except Exception:",
    "            try:",
    "                if __workbook_path.exists():",
    "                    __workbook_path.unlink()",
    "            except Exception:",
    "                pass",
    ...
    "__sas2py_create_combined_csv_workbook(__sas2py_before_files)",
...
function buildRExecutionEnvelope(code: string) {
  ...
  return [
    ...
    "    if (info$size > sas2py_max_artifact_bytes) {",
    "      sas2py_stderr <<- c(sas2py_stderr, paste0('Artifact skipped because it exceeds CODE_RUNNER_MAX_ARTIFACT_BYTES: ', path, ' (', info$size, ' bytes)'))",
    "      next",
    "    }",
    "    current_value <- paste0(info$size, '|', format(info$mtime, '%Y-%m-%dT%H:%M:%OS6', tz = 'UTC'))",
    "    previous_value <- if (path %in% names(before)) before[[path]] else NULL",
    "    if (!is.null(previous_value) && identical(previous_value, current_value)) next",
    "    if (length(artifacts) >= sas2py_max_artifact_count) {",
    "      sas2py_stderr <<- c(sas2py_stderr, paste0('Artifact skipped because CODE_RUNNER_MAX_ARTIFACT_COUNT was reached: ', path))",
    "      break",
    "    }",
    "    if (total_bytes + info$size > sas2py_max_artifact_total_bytes) {",
    "      sas2py_stderr <<- c(sas2py_stderr, paste0('Artifact skipped because CODE_RUNNER_MAX_ARTIFACT_TOTAL_BYTES would be exceeded: ', path, ' (', info$size, ' bytes)'))",
    "      break",
    "    }",
    ...
    "sas2py_last_error_calls <- list()",
    "sas2py_deparse_call <- function(call) {",
    "  paste(deparse(call), collapse = ' ')",
    "}",
    "sas2py_format_error <- function(error, calls) {",
    "  lines <- c(paste0('Error: ', conditionMessage(error)))",
    "  error_call <- conditionCall(error)",
    "  if (!is.null(error_call)) {",
    "    lines <- c(lines, paste0('Error call: ', sas2py_deparse_call(error_call)))",
    "  }",
    "  if (length(calls) > 0) {",
    "    call_text <- vapply(calls, sas2py_deparse_call, character(1))",
    "    call_text <- tail(call_text[nzchar(call_text)], 25)",
    "    if (length(call_text) > 0) {",
    "      lines <- c(lines, 'R call stack, most recent calls:', paste0(seq_along(call_text), ': ', call_text))",
    "    }",
    "  }",
    "  lines",
    "}",
    ...
    "        sas2py_last_error_calls <<- sys.calls()",
    "      }",
    "    )",
    "  },",
    "  error = function(error) {",
    "    sas2py_stderr <<- c(sas2py_stderr, sas2py_format_error(error, sas2py_last_error_calls))",
    "  },",
    "  finally = {",
    ...
    "  }",
    "}",
    "sas2py_artifacts <- tryCatch(",
    "  sas2py_collect_artifacts(sas2py_before_files),",
    "  error = function(error) {",
    "    sas2py_stderr <<- c(sas2py_stderr, paste0('Artifact collection failed: ', conditionMessage(error)))",
    "    list()",
    "  }",
    ")",
